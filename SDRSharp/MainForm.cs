...
namespace SDRSharp
{
    public unsafe partial class MainForm : Form, ISharpControl, INotifyPropertyChanged, IIQProcessor
    {
        ...
        private void InitializeGUI()
        {
            ...
            var stepSizes = Utils.GetStringSetting("stepSizes", "1 Hz,10 Hz,100 Hz,500 Hz,1 kHz,2.5 kHz,5 kHz,6.25 kHz,7.5 kHz,8.33 kHz,9 kHz,10 kHz,12.5 kHz,15 kHz,20 kHz,25 kHz,30 kHz,50 kHz,100 kHz,150 kHz,200 kHz,250 kHz,300 kHz,350 kHz,400 kHz,450 kHz,500 kHz");
            stepSizeComboBox.Items.AddRange(stepSizes.Split(','));
            ...
            MemFreq1.Value = Utils.GetLongSetting("Bookmark1", 0);
            MemFreq2.Value = Utils.GetLongSetting("Bookmark2", 0);
            MemFreq3.Value = Utils.GetLongSetting("Bookmark3", 0);
            MemFreq4.Value = Utils.GetLongSetting("Bookmark4", 0);
            MemFreq5.Value = Utils.GetLongSetting("Bookmark5", 0);
            MemFreq6.Value = Utils.GetLongSetting("Bookmark6", 0);
            MemFreq7.Value = Utils.GetLongSetting("Bookmark7", 0);
            MemFreq8.Value = Utils.GetLongSetting("Bookmark8", 0);
            MemFreq9.Value = Utils.GetLongSetting("Bookmark9", 0);
            MemFreq0.Value = Utils.GetLongSetting("Bookmark0", 0);
            ...
            spectrumAnalyzer.Attack = (float) Utils.GetDoubleSetting("spectrumAnalyzerAttack", 0.9);
            sAttackTrackBar.Value = (int) (spectrumAnalyzer.Attack * sAttackTrackBar.Maximum);

            spectrumAnalyzer.Decay = (float) Utils.GetDoubleSetting("spectrumAnalyzerDecay", 0.3);
            sDecayTrackBar.Value = (int) (spectrumAnalyzer.Decay * sDecayTrackBar.Maximum);

            waterfall.Attack = (float) Utils.GetDoubleSetting("waterfallAttack", 0.9);
            wAttackTrackBar.Value = (int) (waterfall.Attack * wAttackTrackBar.Maximum);

            waterfall.Decay = (float) Utils.GetDoubleSetting("waterfallDecay", 0.5);
        ...
        private void MainForm_Closing(object sender, CancelEventArgs e)
        {
            ...
            Utils.SaveSetting("Bookmark1", MemFreq1.Value);
            Utils.SaveSetting("Bookmark2", MemFreq2.Value);
            Utils.SaveSetting("Bookmark3", MemFreq3.Value);
            Utils.SaveSetting("Bookmark4", MemFreq4.Value);
            Utils.SaveSetting("Bookmark5", MemFreq5.Value);
            Utils.SaveSetting("Bookmark6", MemFreq6.Value);
            Utils.SaveSetting("Bookmark7", MemFreq7.Value);
            Utils.SaveSetting("Bookmark8", MemFreq8.Value);
            Utils.SaveSetting("Bookmark9", MemFreq9.Value);
            Utils.SaveSetting("Bookmark0", MemFreq0.Value);
        }
        ...
        public void Process(Complex* buffer, int length)
        {
            ...
            {
                _fftStream.Write(buffer, length);
        ...
        public void SetFrequency(long frequency, bool onlyMoveCenterFrequency)
        {
            if (onlyMoveCenterFrequency)
            {
                ...
                var delta = frequency - vfoFrequencyEdit.Frequency;
                SetCenterFrequency(_centerFrequency + delta);
                vfoFrequencyEdit.Frequency = frequency;
            }
            else
            {
                Frequency = frequency;
            }
            //FrequencyUpDown.Value = frequency;
        }
        ...
        private void SetCenterFrequency(long newCenterFreq)
        {
            ...
            //FrequencyUpDown.Value = newCenterFreq;
        ...
        private void panview_FrequencyChanged(object sender, FrequencyEventArgs e)
        {
            ...
            FrequencyUpDown.Value = e.Frequency;
        ...
        private void modeRadioButton_CheckStateChanged(object sender, EventArgs e)
        {
            ...
            filterAudioCheckBox.Enabled = !rawRadioButton.Checked;
        ...
        private void stepSizeComboBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var stepSize = 0;
            var match = Regex.Match(stepSizeComboBox.Text, "([0-9\\.]+) ([kMG]?)Hz", RegexOptions.IgnoreCase);
            if (match.Success)
            {
                int multiplier;
                ...
                stepSize = (int)(double.Parse(match.Groups[1].Value, CultureInfo.InvariantCulture) * multiplier);
            }
            if (stepSize > 0)
            {
                waterfall.StepSize = stepSize;
                spectrumAnalyzer.StepSize = stepSize;
                FrequencyUpDown.Increment = stepSize;
        ...
        private void SetModeState(int[] state)
        {
            ...
            stepSizeComboBox.SelectedIndex = Math.Min(stepSizeComboBox.Items.Count - 1, state[7]);
        ...
        {
            spectrumAnalyzer.Attack = sAttackTrackBar.Value / (float) sAttackTrackBar.Maximum;
        ...
        {
            spectrumAnalyzer.Decay = sDecayTrackBar.Value / (float) sDecayTrackBar.Maximum;
        ...
        {
            waterfall.Attack = wAttackTrackBar.Value / (float) wAttackTrackBar.Maximum;
        ...
        {
            waterfall.Decay = wDecayTrackBar.Value / (float) wDecayTrackBar.Maximum;
        ...
        public void GetSpectrumSnapshot(byte[] destArray)
        {
            lock (_fftResolutionLockObject)
            {
                var buffer = new float[destArray.Length];
                fixed (byte* destPtr = destArray)
                fixed (float* bufPtr = buffer)
                {
                    Fourier.SmoothCopy(_fftSpectrumPtr, bufPtr, _fftBins, buffer.Length, 1.0f, 0);
                    Fourier.ScaleFFT(bufPtr, destPtr, buffer.Length, -130.0f, 0.0f);
                }
            }
        }

        public void GetSpectrumSnapshot(float[] destArray, float scale = 1.0f, float offset = 0)
        {
            lock (_fftResolutionLockObject)
            {
                fixed (float* destPtr = destArray)
                {
                    Fourier.SmoothCopy(_fftSpectrumPtr, destPtr, _fftBins, destArray.Length, scale, offset);
                }
            }
        }
        ...
        public void UnregisterStreamHook(object streamHook)
        {
            if (!IsPlaying)
            {
                _vfoHookManager.UnregisterStreamHook(streamHook);
            }
        }
        ...
        {
            if (FrequencyUpDown.Value > vfoFrequencyEdit.Frequency)
            {
                SetFrequency((int)FrequencyUpDown.Value + _stepSize, false);
            }
            if (FrequencyUpDown.Value < vfoFrequencyEdit.Frequency)
            {
                SetFrequency((int)FrequencyUpDown.Value - _stepSize, false);
        ...
        private void waterfall_Load(object sender, EventArgs e)
        {

        }

        private void waterfall_MouseClick(object sender, MouseEventArgs e)
        {
        }
        ...
        private void audioGainTrackBar_Scroll(object sender, EventArgs e)
        {
        ...
        private void spectrumAnalyzer_KeyDown(object sender, KeyEventArgs e)
        {

        }

        private void spectrumAnalyzer_KeyPress(object sender, KeyPressEventArgs e)
        {

        }

        private void fftZoomTrackBar_Scroll(object sender, EventArgs e)
        {

        }

        private void fftContrastTrackBar_Scroll(object sender, EventArgs e)
        {

        }

        private void vfoFrequencyEdit_Load(object sender, EventArgs e)
        {
        ...
        private void fftRangeTrackBar_ValueChanged(object sender, EventArgs e)
        {
            
        }

        private void vfoFrequencyEdit_VisibleChanged(object sender, EventArgs e)
        {
        ...
        private void FrequencyUpDown_Validated(object sender, EventArgs e)
        {
        ...
        private void MainForm_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                SetFrequency((long) MemFreq1.Value, false);
                FrequencyUpDown.Value = MemFreq1.Value;
            }
            if (e.Control && e.KeyCode == Keys.D2)
            {
                SetFrequency((long) MemFreq2.Value, false);
                FrequencyUpDown.Value = MemFreq2.Value;
            }
            if (e.Control && e.KeyCode == Keys.D3)
            {
                SetFrequency((long)MemFreq3.Value, false);
                FrequencyUpDown.Value = MemFreq3.Value;
            }
            if (e.Control && e.KeyCode == Keys.D4)
            {
                SetFrequency((long)MemFreq4.Value, false);
                FrequencyUpDown.Value = MemFreq4.Value;
            }
            if (e.Control && e.KeyCode == Keys.D5)
            {
                SetFrequency((long)MemFreq5.Value, false);
                FrequencyUpDown.Value = MemFreq5.Value;
            }
            if (e.Control && e.KeyCode == Keys.D6)
            {
                SetFrequency((long)MemFreq6.Value, false);
                FrequencyUpDown.Value = MemFreq6.Value;
            }
            if (e.Control && e.KeyCode == Keys.D7)
            {
                SetFrequency((long)MemFreq7.Value, false);
                FrequencyUpDown.Value = MemFreq7.Value;
            }
            if (e.Control && e.KeyCode == Keys.D8)
            {
                SetFrequency((long)MemFreq8.Value, false);
                FrequencyUpDown.Value = MemFreq8.Value;
            }
            if (e.Control && e.KeyCode == Keys.D9)
            {
                SetFrequency((long)MemFreq9.Value, false);
                FrequencyUpDown.Value = MemFreq9.Value;
            }
            if (e.Control && e.KeyCode == Keys.D0)
            {
                SetFrequency((long)MemFreq0.Value, false);
                FrequencyUpDown.Value = MemFreq0.Value;
            }

            if (e.Alt && e.KeyCode == Keys.D0)
            {
                MemFreq0.Value = FrequencyUpDown.Value;
            }
            if (e.Alt && e.KeyCode == Keys.D1)
            {
                MemFreq1.Value = FrequencyUpDown.Value;
            }
            if (e.Alt && e.KeyCode == Keys.D2)
            {
                MemFreq2.Value = FrequencyUpDown.Value;
            }
            if (e.Alt && e.KeyCode == Keys.D3)
            {
                MemFreq3.Value = FrequencyUpDown.Value;
            }
            if (e.Alt && e.KeyCode == Keys.D4)
            {
                MemFreq4.Value = FrequencyUpDown.Value;
            }
            if (e.Alt && e.KeyCode == Keys.D5)
            {
                MemFreq5.Value = FrequencyUpDown.Value;
            }
            if (e.Alt && e.KeyCode == Keys.D6)
            {
                MemFreq6.Value = FrequencyUpDown.Value;
            }
            if (e.Alt && e.KeyCode == Keys.D7)
            {
                MemFreq7.Value = FrequencyUpDown.Value;
            }
            if (e.Alt && e.KeyCode == Keys.D8)
            {
                MemFreq8.Value = FrequencyUpDown.Value;
            }
            if (e.Alt && e.KeyCode == Keys.D9)
            {
                MemFreq9.Value = FrequencyUpDown.Value;
            }
            if (e.Alt && e.KeyCode == Keys.D0)
            {
                MemFreq0.Value = FrequencyUpDown.Value;
            ...
            /*a and A = Shift Frequency Left */
            if (e.KeyCode == Keys.A)
            {
                SetFrequency(vfoFrequencyEdit.Frequency - spectrumAnalyzer.StepSize, false);
                FrequencyUpDown.Value = vfoFrequencyEdit.Frequency;
            }
            /*d and D = Shift Frequency Right */
            if (e.KeyCode == Keys.D)
            {
                SetFrequency(vfoFrequencyEdit.Frequency + spectrumAnalyzer.StepSize, false);
                FrequencyUpDown.Value = vfoFrequencyEdit.Frequency;
        ...
        private void MemFreq1_ValueChanged(object sender, EventArgs e)
        {

        }

        private void MemFreq0_ValueChanged(object sender, EventArgs e)
        {
