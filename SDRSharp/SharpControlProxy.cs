...
namespace SDRSharp
{
    public class SharpControlProxy : ISharpControl, INotifyPropertyChanged
    {
        ...
        public void SetFrequency(long frequency, bool onlyMoveCenterFrequency)
        {
            if (_owner.InvokeRequired)
            {
                _owner.Invoke(new MethodInvoker(() => _owner.SetFrequency(frequency, onlyMoveCenterFrequency)));
            }
            else
            {
                _owner.SetFrequency(frequency, onlyMoveCenterFrequency);
            }
        }
        ...
        public void GetSpectrumSnapshot(float[] destArray, float scale = 1.0f, float offset = 0.0f)
        {
            _owner.GetSpectrumSnapshot(destArray, scale, offset);
        }
