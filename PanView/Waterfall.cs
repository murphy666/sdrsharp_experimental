...
namespace SDRSharp.PanView
{
    ...
    public class Waterfall : UserControl
    {
        ...
        public Waterfall()
        {
            _smoothedSpectrum = new float[ClientRectangle.Width - 2 * AxisMargin];
            _scaledSpectrum = new byte[_smoothedSpectrum.Length];
            _temp = new float[_smoothedSpectrum.Length];
        ...
        {
            var offset = (float) (_displayCenterFrequency - _centerFrequency) / (float) _spectrumWidth;
            ExtractSpectrum(powerSpectrum, length, offset, _scale);
            Draw();
            Invalidate();
        }

        private unsafe void ExtractSpectrum(float* powerSpectrum, int length, float offset, float scale)
        {
            var displayOffset = _displayOffset / 10 * 10;
            var displayRange = _displayRange / 10 * 10;

            fixed (float* temp = _temp)
            fixed (float* smoothedSpectrum = _smoothedSpectrum)
            fixed (byte* scaledSpectrum = _scaledSpectrum)
            {
                Fourier.SmoothCopy(powerSpectrum, temp, length, _smoothedSpectrum.Length, scale, offset);

                if (_useSmoothing)
                {
                    for (var i = 0; i < _temp.Length; i++)
                    {
                        var ratio = smoothedSpectrum[i] < temp[i] ? Attack : Decay;
                        smoothedSpectrum[i] = smoothedSpectrum[i] * (1 - ratio) + temp[i] * ratio;
                    }
                }
                else
                {
                    Utils.Memcpy(smoothedSpectrum, temp, _smoothedSpectrum.Length * sizeof(float));
                }
                Fourier.ScaleFFT(smoothedSpectrum, scaledSpectrum, _smoothedSpectrum.Length, displayOffset - displayRange, displayOffset);
            }
        }
        ...
        private void Draw()
        {
            ...
            #region Shift image and draw Spectrum
        ...
        private unsafe void InsertNewLine()
        {
            ...
            if (_scaledSpectrum != null && _scaledSpectrum.Length > 0)
            {
                var ptr = (int*) src + AxisMargin;

                for (var i = 0; i < _scaledSpectrum.Length; i++)
                {
                    var colorIndex = (int)((_scaledSpectrum[i] + _contrast * 50.0 / 25.0) * _gradientPixels.Length / byte.MaxValue);
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            _scaledSpectrum = new byte[length];
            _smoothedSpectrum = new float[length];
            _temp = new float[length];
