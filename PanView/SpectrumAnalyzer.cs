...
namespace SDRSharp.PanView
{
    public class SpectrumAnalyzer : UserControl
    {
        ...
        public SpectrumAnalyzer()
        {
            ...
            var defaultLevel = - (_displayOffset + _displayRange);
            for (var i = 0; i < length; i++)
            {
                _smoothedSpectrum[i] = defaultLevel;
            }
        ...
        public unsafe void Render(float* powerSpectrum, int length)
        {
            ...
            ExtractSpectrum(powerSpectrum, length, offset, _scale, _useSmoothing);
            _performNeeded = true;
        }
        ...
        private void DrawSpectrum()
        {
            ...
            var xIncrement = (ClientRectangle.Width - 2 * AxisMargin) / (float) _scaledSpectrum.Length;
            var yIncrement = (ClientRectangle.Height - 2 * AxisMargin) / (float) byte.MaxValue;
            ...
            using (var spectrumPen = new Pen(_spectrumColor))
            {
                ...
                {
                    var strenght = _scaledSpectrum[i];
        ...
        protected override unsafe void OnResize(EventArgs e)
        {
            ...
            if (ClientRectangle.Width > 0 && ClientRectangle.Height > 0)
            {
                ...
                _bkgBuffer = new Bitmap(ClientRectangle.Width, ClientRectangle.Height, PixelFormat.Format32bppPArgb);
                var length = ClientRectangle.Width - 2 * AxisMargin;
                var oldSpectrum = _smoothedSpectrum;
                _scaledSpectrum = new byte[length];
                _smoothedSpectrum = new float[length];
                _temp = new float[length];
                _peaks = new bool[length];
                fixed (float *old = oldSpectrum)
                {
                    ExtractSpectrum(old, oldSpectrum.Length, 0.0f, 1.0f, false);
                }

                _points = new Point[length + 2];
                if (_spectrumWidth > 0)
                {
                    _xIncrement = _scale * length / _spectrumWidth;
                }
